...
//  onemoretimer
//
//  Created by jongchan on 2021/02/24.
//

import UIKit

class LiftingProgressBar: UIView {
    ...
    func loadProgress(percentage: CGFloat) {
        ...
        basicAnimation.duration = CFTimeInterval(LiftingTimerViewController.timeOut) // 끝나는점(timeOut)
    ...
    func loadProgressPause(percentage: CGFloat, nowTime: Double) {
        ...
        let basicAnimation = CABasicAnimation(keyPath: "strokeEnd")
        basicAnimation.fromValue = nowTime / Double(LiftingTimerViewController.timeOut)
        basicAnimation.duration = Double(LiftingTimerViewController.timeOut) // 프로그래스바 일시정지하면 멈춰진 시간에 정지
    ...
    func loadProgressRestart(percentage: CGFloat,nowTime: Double) {
        ...
        let basicAnimation = CABasicAnimation(keyPath: "strokeEnd")
        basicAnimation.fromValue = nowTime / Double(LiftingTimerViewController.timeOut)
        basicAnimation.duration = Double(LiftingTimerViewController.timeOut) - nowTime // restart하면 현재 시간부터 프로그래스바 실행
        basicAnimation.fillMode = CAMediaTimingFillMode.forwards
        basicAnimation.isRemovedOnCompletion = false
